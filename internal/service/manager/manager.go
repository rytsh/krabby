...
// BackfillGraphs rebuilds tracked graphs that predate the managed ignore file or
// were generated by a different Graphify version. Work is submitted to the
// bounded background queue and never blocks startup.
func (m *Manager) BackfillGraphs(ctx context.Context) {
	repos, err := m.reg.List(ctx)
	if err != nil {
		slog.Error("list repos for graph metadata backfill", "error", err)
	...
	for _, repo := range repos {
		...
		if graphify.HasManagedIgnore(repo.Path) && m.gfy.GraphBuiltWithCurrentVersion(repo.Path) {
			continue
		}
	...
	for _, name := range generateOrder {
		...
		case registry.StageDocsIndex:
			serr = m.runStage(ctx, repo, name, func() error {
				...
				return m.indexDocs(ctx, d, repo.ID, docsDir)
			})
		}
...
func (m *Manager) refresh(ctx context.Context, repo *registry.Repo) error {
	...
	if staleVersion && snapshot.Commit == repo.LastCommit {
		slog.Info("graph was built by a different Graphify version; rebuilding",
			"repo", repo.ID, "graphify_version", m.gfy.Version())
	}
...
func (m *Manager) rebuildMerged(ctx context.Context) error {
	...
	case 1:
		...
		if err := graphquery.Validate(graphs[0]); err != nil {
			return fmt.Errorf("validate graphify output before merge; %w", err)
		}
	...
	default:
		...
		if err := graphquery.Validate(tmp); err != nil {
			_ = os.Remove(tmp)
			return fmt.Errorf("validate merged graphify output; %w", err)
		}
...
// GraphifyVersion returns the version reported by the configured Graphify CLI.
func (m *Manager) GraphifyVersion() string {
	if m.gfy == nil {
		return "unknown"
	}
