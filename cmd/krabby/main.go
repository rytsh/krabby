...
func run(ctx context.Context) error {
	...
	defer cancelApp()
	background, appCtx := errgroup.WithContext(appCtx)

	mgr := manager.New(appCtx, reg, git, gfy, engine, creds, codeText, cfg.ReposDir(), cfg.MergedGraphPath(),
		cfg.Graphify.Merge,
		manager.DocsDeps{
			TextStore:      docsText,
	...
	// Join application-owned goroutines before Manager.Close releases their
	// stores, and before the state database underneath the manager is closed.
	defer func() {
		cancelApp()
		if err := background.Wait(); err != nil {
			slog.Error("wait for application background work", "error", err)
		}
	}()
	...
	background.Go(func() error {
		if err := mgr.WarmCodeSearch(appCtx); err != nil && appCtx.Err() == nil {
			slog.Error("warm normal code search index", "error", err)
		}
		if err := mgr.WarmDocsSearch(appCtx); err != nil && appCtx.Err() == nil {
	...
	// Re-enqueue background tasks that were queued (or running) when the process
	// last stopped. Restore lists every record and seeds the sequence allocator
	// before enabling any new durable submission. Dispatch remains paused until
	// the startup-generated work below has had a chance to deduplicate against
	// the restored backlog.
	if err := mgr.RestoreTasks(ctx); err != nil {
		return fmt.Errorf("restore persisted background tasks; %w", err)
	}
	// Rebuild graphs that predate managed ignores or were generated by a
	// different Graphify release. This must follow RestoreTasks so its sequence
	// cannot collide with an unseen durable record.
	...
	if projectionUpgrade != nil {
		...
		if err := mgr.TriggerReindexAll(); err != nil {
			slog.Error("persist docs index projection reindex", "error", err)
	...
	if err := server.Start(appCtx, cfg, mgr, mcpServer, mcpAPIServer, mcpAdminServer); err != nil {
